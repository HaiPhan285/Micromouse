...
static constexpr uint32_t kSampleTimeUs = 100'000; // Sample time for velocity calculation (100ms)
...
int main() {
  ...
  while (1) {
    EncoderStats stats =
        hw.encoder.measure_encoder_stats(sample_cycles, kTicksPerOutputRev, kCmPerTick);
