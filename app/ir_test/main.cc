...
int main(int argc, char* argv[]) {
  ...
  while (1) {
    ...
    int msg_len = std::snprintf(reinterpret_cast<char*>(tx_buf.data()), tx_buf.size(),
                                "L:%u FL:%u FR:%u R:%u\r\n", vals.left, vals.front_left,
                                vals.front_right, vals.right);

    if (msg_len > 0) {
      const size_t len = static_cast<size_t>(msg_len);
      board.usart.send(
          std::span<const uint8_t>(tx_buf.data(), (len < tx_buf.size()) ? len : tx_buf.size()));
    }
