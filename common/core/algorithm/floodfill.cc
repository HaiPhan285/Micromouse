...
bool Floodfill::track_path()
{
    ...
    const bool in_goal_x{current_x == 7 || current_x == 8};
    const bool in_goal_y{current_y == 7 || current_y == 8};
    searched = searched || (in_goal_x && in_goal_y);
...
bool Floodfill::is_searched() const
{
    return searched;
}
...
void Floodfill::process_ir_data(const IrValues& ir_vals)
{
    ...
    const bool wall_front{(ir_vals.front_left > kThresholdFront) ||
                          (ir_vals.front_right > kThresholdFront)};
    const bool wall_right{ir_vals.right > kThresholdSide};
    const bool wall_left{ir_vals.left > kThresholdSide};
